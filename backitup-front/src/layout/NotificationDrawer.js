...
const NotificationDrawer = ({ show, onClose, currUser }) => {
  ...
  const loadNotif = async () => {
    try {
      const result =
        await axios.get(`https://orbital-1690146023037.azurewebsites.net/api/listNotification/unread/${currUser.userID}`)
      setNotifs(result.data);
      const result2 =
        await axios.get(`https://orbital-1690146023037.azurewebsites.net/api/listNotification/read/${currUser.userID}`)
