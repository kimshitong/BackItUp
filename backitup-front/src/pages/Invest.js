...
export default function Invest(props) {
  ...
  const loadUser = async () => {
    ...
    const currUser = await axios.get(`https://orbital-1690142964708.azurewebsites.net/api/user/${props.isAuth.userID}`)
  ...
  const onSubmit = async (event) => {
    ...
    const result =
      await axios.get(`https://orbital-1690142964708.azurewebsites.net/api/invest/${id}/${props.isAuth.userID}/${amt}/${formattedDate}`)
    if (result.data > 0) {
      navigate("/thanks")
    } else {
      result.data === -1
