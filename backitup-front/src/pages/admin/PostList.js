...
export default function PostList() {
  ...
  const loadPosts = async () => {
    const result = await axios.get("https://orbital-1690142964708.azurewebsites.net/api/listPosts")
    console.log(result);
  ...
  const clickVerify = (postID) => {
    ...
    axios.get(`https://orbital-1690142964708.azurewebsites.net/api/post/verify/${postID}/${formattedDate}`)
  ...
  const clickUnverify = (postID) => {
    const date = new Date();
    const formattedDate = date.toISOString().substr(0, 19);
    axios.get(`https://orbital-1690142964708.azurewebsites.net/api/post/unverify/${postID}/${formattedDate}`)
