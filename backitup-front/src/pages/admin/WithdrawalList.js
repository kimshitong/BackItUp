...
export default function WithdrawalList() {
    ...
    const loadWds = async () => {
        const result = await axios.get("http://orbital-1687676297440.azurewebsites.net/api/listWithdrawal")
        console.log(result);
        setWds(result.data)
    ...
    const clickVerify = (withdrawalID) => {
        ...
        axios.get(`http://orbital-1687676297440.azurewebsites.net/api/withdrawal/verify/${withdrawalID}/${dt}`)
    ...
    const clickUnverify = (withdrawalID) => {
        const date = new Date();
        const dt = date.toISOString().substr(0, 19);
        axios.get(`http://orbital-1687676297440.azurewebsites.net/api/withdrawal/unverify/${withdrawalID}/${dt}`)
